...
func (opp *KeyUpdaterProcessor) PreProcess(
	...
	if st, err := state.ExistsState(currency.StateKeyAccount(fact.target), "target keys", getStateFunc); err != nil {
		return ctx, base.NewBaseOperationProcessReasonError("failed to check existence of target %v : %v", fact.target, err), nil
	} else if err := state.CheckNotExistsState(extension.StateKeyContractAccount(fact.Target()), getStateFunc); err != nil {
		return ctx, base.NewBaseOperationProcessReasonError("contract account not allowed for key updater, %q: %v", fact.Target(), err), nil
	} else if ks, err := currency.StateKeysValue(st); err != nil {
		return ctx, base.NewBaseOperationProcessReasonError("failed to get state value of keys %v : %v", fact.keys.Hash(), err), nil
	} else if ks.Equal(fact.Keys()) {
		return ctx, base.NewBaseOperationProcessReasonError("same Keys as existing %v : %v", fact.keys.Hash(), err), nil
	}
...
func (opp *KeyUpdaterProcessor) Process( // nolint:dupl
	...
	if tgAccSt, err = state.ExistsState(currency.StateKeyAccount(fact.target), "target keys", getStateFunc); err != nil {
		return nil, base.NewBaseOperationProcessReasonError("failed to check existence of target %v : %v", fact.target, err), nil
	}
	...
	if policy, err = state.ExistsCurrencyPolicy(fact.currency, getStateFunc); err != nil {
		...
		return nil, base.NewBaseOperationProcessReasonError("failed to check fee of currency %v : %v", fact.currency, err), nil
	}
	...
	if tgBalSt, err = state.ExistsState(currency.StateKeyBalance(fact.target, fact.currency), "balance of target", getStateFunc); err != nil {
		return nil, base.NewBaseOperationProcessReasonError("failed to check existence of targe balance %v : %v", fact.target, err), nil
	} else if b, err := currency.StateBalanceValue(tgBalSt); err != nil {
		return nil, base.NewBaseOperationProcessReasonError("failed to check existence of target balance %v,%v : %v", fact.currency, fact.target, err), nil
