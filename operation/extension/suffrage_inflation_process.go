...
//}
//
//func NewSuffrageInflationProcessor(threshold mitumbase.Threshold) GetNewProcessor {
...
//		item := fact.Items()[i]
//
//		err := state.CheckExistsState(statecurrency.StateKeyCurrencyDesign(item.Amount().Currency()), getStateFunc)
//		if err != nil {
//			return ctx, mitumbase.NewBaseOperationProcessReasonError("currency not found, %q: %w", item.Amount().Currency(), err.Error()), nil
//		}
//
//		err = state.CheckExistsState(statecurrency.StateKeyAccount(item.Receiver()), getStateFunc)
//		if err != nil {
...
//		var ab base.Amount
//
//		k := statecurrency.StateKeyBalance(item.Receiver(), item.Amount().Currency())
...
//		default:
//			b, err := statecurrency.StateBalanceValue(st)
//			if err != nil {
...
//		}
//
//		sts = append(sts, state.NewStateMergeValue(k, statecurrency.NewBalanceStateValue(base.NewAmount(ab.Big().Add(item.Amount().Big()), item.Amount().Currency()))))
//
...
//
//		k := statecurrency.StateKeyCurrencyDesign(cid)
//		switch st, found, err := getStateFunc(k); {
//		case err != nil:
...
//		default:
//			d, err := statecurrency.StateCurrencyDesignValue(st)
//			if err != nil {
...
//		}
//
//		sts = append(sts, state.NewStateMergeValue(k, statecurrency.NewCurrencyDesignStateValue(ade)))
//	}
//
//	return sts, nil, nil
//}
